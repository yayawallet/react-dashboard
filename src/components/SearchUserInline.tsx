      ...
      {usersList.length > 0 ? (
        <div className="bg-gray-0 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none">
          {usersList?.slice(0, 5).map((user) => (
            <div
              key={user.account}
              className={`flex gap-2 items-center px-2 py-1 border-b border-gray-100 hover:bg-gray-100 cursor-pointer wborder rounded ${selectedUser ? 'bg-yaya-500 hover:bg-yaya-500 rounded text-white' : ''}`}
              onClick={() => {
