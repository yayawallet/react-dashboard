...
const ApprovalRequestsList = () => {
  ...
  } = useGetData(`/transaction/transaction-requests?page=${currentPage}`);
  ...
  const handleApproveApprovalRequest = (confirm: boolean) => {
    ...
    authAxios
      .post('transaction/submit-transaction-response', formData)
      .then(() => {
                  ...
                  <tbody>
                    {approvalRequestList.map((t: ApprovalRequesType) => (
                      <React.Fragment key={t.uuid}>
                        <tr
                          className="hover:bg-slate-100 text-nowrap"
